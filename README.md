...
the withdraw path (DEV/TEST trusted setup only). Umbra SDK is installed and
fail-closed in the frontend for supported SPL/Token-2022 exits; the current C2H
native SOL withdrawal path remains direct `stealth_address` mode. Other external
privacy rails (IKA, MagicBlock PER/Private Payments, Encrypt/FHE) are not yet wired.
...
| External privacy rails | Partially wired | Umbra SDK adapter installed/fail-closed; IKA, PER, Private Payments, Encrypt/FHE not wired |
