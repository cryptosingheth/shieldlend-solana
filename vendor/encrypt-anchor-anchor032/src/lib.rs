...
//! Anchor 0.32.x CPI SDK for the Encrypt program.
//!
//! This is a minimal local compatibility fork of the official
//! `chains/solana/program-sdk/anchor` crate from
//! `dwallet-labs/encrypt-pre-alpha`, rebased onto Anchor 0.32.1 so ShieldLend
//! can test the CPI boundary without downgrading the workspace.
...
impl<'info> EncryptContext<'info> {
    pub fn execute_graph(&self, ix_data: &[u8], remaining: &[AccountInfo<'info>]) -> Result<()> {
        let mut accounts = vec![
            AccountMeta::new(self.config.key(), false),
            AccountMeta::new(self.deposit.key(), false),
            AccountMeta::new_readonly(self.caller_program.key(), false),
    ...
    pub fn make_public(&self, ciphertext: &AccountInfo<'info>) -> Result<()> {
