...
| **MagicBlock** | TEE RPC `https://devnet-tee.magicblock.app` HTTP 200. Router RPC `https://devnet-router.magicblock.app` HTTP 200. PER sidecar TypeScript builders: 4 ShieldLend use-case bundles, 17/17 pass. Private Payments API `https://payments.magicblock.app` health/challenge/login/mint/balance/builders live; wSOL deposit and withdraw signed/submitted on devnet. The private-transfer harness now performs SOL -> wSOL, auth, mint check, deposit, balance polling, then transfer attempt. | Rust PER macros in Anchor programs (blocked: Anchor 0.30.1 vs 0.32.1 required). Private transfer is not live: after deposit, authenticated private-balance polling does not show sufficient private wSOL credit, and transfer execution fails with Token Program `0x1` InsufficientFunds; router also returns `Blockhash not found`. TDX attestation challenge format mismatch with SDK 0.8.8. |
...
- MagicBlock Private Payments private-transfer harness now funds wSOL, logs in, checks mint initialization, deposits wSOL, polls public/private balances, and attempts transfer in the same owner/mint context.
...
- MagicBlock Private Payments private transfer through the intended ephemeral/router path (builder returns 200, but the funded path lacks a verified private wSOL credit after deposit and transfer execution fails with Token Program `0x1` InsufficientFunds; router submit also blocks with `Blockhash not found`).
...
| MagicBlock Private Payments private transfer balance | `--live-private-transfer` now runs SOL -> wSOL, login, mint check, deposit, private-balance polling, then transfer. The deposit tx submits, but authenticated private-balance polling for the same owner/mint returns `balance: "0"` and `location: "base"`; transfer then fails with Token Program `0x1` InsufficientFunds. | Confirm with MagicBlock whether deposit is meant to credit a different private-balance namespace/account context or whether the public API lacks the required private-transfer router path |
