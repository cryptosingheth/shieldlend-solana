...
```
   ...
   UNREGISTERED
      ...
      [end — note is gone]                                                 ▼
                              ...
                              repaymentAmount ≥ outstanding. Health factor confirmed breached
...
**Activates when**: All protocol dependencies operational (MagicBlock PER, IKA dWallet, Encrypt FHE)
...
**Activates when**: `ShieldedPoolState.epochs_without_per_flush >= per_fallback_epoch_threshold`
...
flush_epoch called (IKA relay, PER-authorized)
    ...
    ├─ 3. Generate VRF dummy commitments: Poseidon(vrf_output_i, denomination)
...
**VRF dummy formula**: `dummy_commitment = Poseidon(vrf_output_i, denomination)`
- `vrf_output_i`: `VRF_PROOF.output || epoch_index || dummy_index`, hashed to 256 bits
- `denomination`: same fixed set as real commitments

**Why this is indistinguishable**: The VRF output is deterministic given the VRF key and input, but the VRF key is held by MagicBlock's VRF contract. An external observer sees only `dummy_commitment` — a 256-bit field element. Without knowing `vrf_output_i`, the observer cannot distinguish this from `Poseidon(secret, nullifier, denomination)` for unknown secret and nullifier.

**What the observer CAN compute**: Given the on-chain VRF proof, they can verify that `vrf_output` was generated from the correct input. But they still cannot extract `vrf_output_i` used in each dummy commitment without the VRF contract's secret.
