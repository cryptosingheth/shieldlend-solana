...
Last reconciled: 2026-05-09 (live/encrypt-anchor: Encrypt Anchor compatibility boundary)
...
| MagicBlock Private Payments private transfer submit | API returns unsigned `sendTo=ephemeral` private-transfer transaction. Decoded tx blockhash matches API blockhash, but router returns `Blockhash not found` and TEE rejects writable accounts. Base devnet accepts the refreshed tx only — not the intended ephemeral route. Deposit/withdraw remain live on base devnet. |
...
| Private Payments private transfer submit | `POST /v1/spl/transfer` returns an unsigned `sendTo=ephemeral` transaction. Router submit fails with `Blockhash not found`; TEE submit fails with writable-account verification; base devnet accepts locally refreshed transaction only — not the intended ephemeral route. | Confirm correct ephemeral submit RPC or API blockhash behavior with MagicBlock |
