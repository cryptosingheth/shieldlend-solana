...
| External privacy rails | Umbra SDK funded wSOL deposit/withdraw confirmed; Encrypt gRPC CreateInput confirmed; MagicBlock TEE RPC reachable + TypeScript PER adapter live; MagicBlock Private Payments API + wSOL deposit/withdraw live on devnet; MagicBlock private-transfer harness now funds/deposits/checks balances and retries the documented `base -> ephemeral` route before transfer; IKA SDK/capability probe confirmed | IKA relay signing, ShieldLend-native Umbra payout, PER macros (Anchor 0.32.1 blocked), Private Payments private transfer through the intended ephemeral/router RPC, usable private wSOL balance credit after deposit/top-up, and Encrypt/FHE on-chain health computation are not live |
